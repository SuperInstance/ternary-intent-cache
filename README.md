...
Cache for intent→bytecode compilations. When the same intent is expressed repeatedly, return the cached Flux bytecode. LRU eviction with TTL.
...
```bash
git clone https://github.com/SuperInstance/ternary-intent-cache.git
cd ternary-intent-cache
cargo test    # 6 tests
```
